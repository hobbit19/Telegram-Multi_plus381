...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public MessagesController() {
        ...
        addSupportUser();
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);        enableJoined = preferences.getBoolean("EnableContactJoined", true);
        //plus
        ...
        //
        preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);        secretWebpagePreview = preferences.getInt("secretWebpage2", 2);
    ...
    public void updateConfig(final TLRPC.TL_config config) {
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                ...
                SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);                SharedPreferences.Editor editor = preferences.edit();
    ...
    public void hideReportSpam(final long dialogId, TLRPC.User currentUser, TLRPC.Chat currentChat) {
        ...
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);        SharedPreferences.Editor editor = preferences.edit();
    ...
    public void loadPeerSettings(final long dialogId, TLRPC.User currentUser, TLRPC.Chat currentChat) {
        ...
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);        if (preferences.getInt("spam3_" + dialogId, 0) == 1) {
            return;
        }
    ...
    private void applyDialogNotificationsSettings(long dialog_id, TLRPC.PeerNotifySettings notify_settings) {
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);        int currentValue = preferences.getInt("notify2_" + dialog_id, 0);
    ...
    private void applyDialogsNotificationsSettings(ArrayList<TLRPC.Dialog> dialogs) {
        ...
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            if (dialog.peer != null && dialog.notify_settings instanceof TLRPC.TL_peerNotifySettings) {
                if (editor == null) {
                    SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);                    editor = preferences.edit();
                }
                int dialog_id;
                ...
                if (!updatesOnMainThread.isEmpty()) {
                    ...
                    for (int a = 0; a < updatesOnMainThread.size(); a++) {
                        ...
                        if (update instanceof TLRPC.TL_updatePrivacy) {
                            ...
                            SharedPreferences.Editor editor2 = ApplicationLoader.applicationContext.getSharedPreferences("emoji" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE).edit();                            editor2.putLong("lastGifLoadTime", 0).commit();
                        }
                    }
                    if (editor != null) {
                        editor.commit();
    ...
    public boolean isDialogMuted(long dialog_id) {
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications" + ChangeUserHelper.getUserTag(), Activity.MODE_PRIVATE);        int mute_type = preferences.getInt("notify2_" + dialog_id, 0);
        if (mute_type == 2) {
            return true;
        } else if (mute_type == 3) {
